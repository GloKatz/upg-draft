...
<head>
  ...
  <script>
    ...
    var mkdocs_page_input_path = "node_modules\\dashdash\\README.md";
...
<pre><code class="javascript">var dashdash = require('dashdash');
...
<pre><code class="javascript">var dashdash = require('./lib/dashdash');
...
option:</p>
<pre><code class="shell">$ FOO_VERBOSE=1 node foo.js
# opts: { verbose: [ true ],
...
<pre><code class="shell">$ FOO_VERBOSE= node examples/foo.js                 # not set
# opts: { _order: [], _args: [] }
# args: []
...
<pre><code class="shell">$ FOO_FILE=data.txt node examples/foo.js
# opts: { file: 'data.txt',
...
<p>Numbers:</p>
<pre><code class="shell">$ FOO_TIMEOUT=5000 node examples/foo.js
# opts: { timeout: 5000,
...
<p>Set it to false to have '-h' <strong>not</strong> get parsed as an option in the above
  example.</p>
<ul>
<li><code>allowUnknown</code> (Boolean).  Optional.  Default is false.  If false, this causes
  unknown arguments to throw an error.  I.e.:<pre><code>node ./tool.js -v arg1 --afe8asefksjefhas
</code></pre>
</li>
</ul>
...
<p>where <code>-x</code> and <code>-z</code> are unknown.  There are multiple ways to
  interpret this:</p>
<pre><code>1. `-x` takes a value: `{x: 'az'}`
