...
<head>
  ...
  <title>node-dashdash changelog - DCP</title>
  ...
  <script>
    // Current page data
    var mkdocs_page_name = "node-dashdash changelog";
    var mkdocs_page_input_path = "node_modules\\dashdash\\CHANGES.md";
...
<ul>
<li>[issue #30] Change the output used by dashdash's Bash completion support to
  indicate "there are no completions for this argument" to cope with different
  sorting rules on different Bash/platforms. For example:<pre><code>$ triton -v -p test2 package get &lt;TAB&gt;          # before
...
<ul>
<li>New <code>synopsisFromOpt(&lt;option spec&gt;)</code> function. This will be used by
  <a href="https://github.com/trentm/node-cmdln">node-cmdln</a> to put together a synopsis
  of options for a command. Some examples:<pre><code>&gt; synopsisFromOpt({names: ['help', 'h'], type: 'bool'});
'[ --help | -h ]'
...
<ul>
<li>Add the <code>arrayFlatten</code> boolean option to <code>dashdash.addOptionType</code> used for
  custom option types. This allows one to create an <code>arrayOf...</code> option type
  where each usage of the option can return multiple results. For example:<pre><code>node mytool.js --foo a,b --foo c
</code></pre>
</li>
</ul>
...
<h2 id="173">1.7.3</h2>
<ul>
<li>[issue #8] Fix parsing of a short option group when one of the
  ...
  argument. This should parse:<pre><code>tail -fn5
</code></pre>
</li>
</ul>
...
<ul>
<li>[issue #4] Add <code>date</code> and <code>arrayOfDate</code> option types. They accept these date
  formats: epoch second times (e.g. 1396031701) and ISO 8601 format:
  <code>YYYY-MM-DD[THH:MM:SS[.sss][Z]]</code> (e.g. "2014-03-28",
  "2014-03-28T18:35:01.489Z"). See "examples/date.js" for an example usage.<pre><code>$ node examples/date.js -s 2014-01-01 -e $(date +%s)
...
<h2 id="132">1.3.2</h2>
<ul>
<li>Fix a subtlety where a <em>bool</em> option using both <code>env</code> and <code>default</code> didn't
  work exactly correctly. If <code>default: false</code> then all was fine (by luck).
  However, if you had an option like this:<pre><code>options: [ {
    names: ['verbose', 'v'],
    env: 'FOO_VERBOSE',
    'default': true,    // &lt;--- this
    type: 'bool'
...
<h2 id="131">1.3.1</h2>
<ul>
<li>[issue #1] Fix an envvar not winning over an option 'default'. Previously
  an option with both <code>default</code> and <code>env</code> would never take a value from the
  environment variable. E.g. <code>FOO_FILE</code> would never work here:<pre><code>options: [ {
    names: ['file', 'f'],
    env: 'FOO_FILE',
...
<h2 id="130">1.3.0</h2>
<ul>
<li>[Backward incompatible change for boolean envvars] Change the
  ...
  true -- which was quite misleading. Example:<pre><code>$ FOO_VERBOSE=0 node examples/foo.js
# opts: { verbose: [ false ],
...
<h2 id="121">1.2.1</h2>
<ul>
<li>Fix for <code>parse.help({includeEnv: true, ...})</code> handling to ensure that an
  option with an <code>env</code> <strong>but no <code>help</code></strong> still has the "Environment: ..."
  output. E.g.:<pre><code>{ names: ['foo'], type: 'string', env: 'FOO' }
