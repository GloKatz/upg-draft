...
<head>
  ...
  <script>
    // Current page data
    var mkdocs_page_name = "Commander.js";
    var mkdocs_page_input_path = "node_modules\\commander\\Readme.md";
...
<pre><code class="js">function range(val) {
...
<pre><code class="js">// file: ./examples/pm
var program = require('commander');
...
<p>You can enable <code>--harmony</code> option in two ways:
* Use <code>#! /usr/bin/env node --harmony</code> in the sub-commands scripts. Note some os version don’t support this pattern.
* Use the <code>--harmony</code> option when call the command, like <code>node --harmony examples/pm publish</code>. The <code>--harmony</code> option will be preserved when spawning sub-command process.</p>
...
<pre><code class="js">var program = require('commander');
