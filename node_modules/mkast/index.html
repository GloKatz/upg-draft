...
<head>
  ...
  <script>
    // Current page data
    var mkdocs_page_name = "Tree";
    var mkdocs_page_input_path = "node_modules\\mkast\\README.md";
  ...
  <hr/>
          ...
          <div role="main">
            <div class="section">
                ...
                <h1 id="tree">Tree</h1>
<p><a href="https://travis-ci.org/mkdoc/mkast"><img alt="Build Status" src="https://travis-ci.org/mkdoc/mkast.svg?v=3" /></a>
<a href="https://npmjs.org/package/mkast"><img alt="npm version" src="http://img.shields.io/npm/v/mkast.svg?v=3" /></a>
<a href="https://coveralls.io/github/mkdoc/mkast?branch=master"><img alt="Coverage Status" src="https://coveralls.io/repos/mkdoc/mkast/badge.svg?branch=master&amp;service=github&amp;v=3" /></a></p>
<blockquote>
<p>Converts nodes to and from JSON</p>
</blockquote>
<p>Transforms <a href="http://commonmark.org">commonmark</a> nodes to and from JSON for piping between processes.</p>
<h2 id="install">Install</h2>
<pre><code>npm i mkast
</code></pre>
...
<p>Serialize <a href="http://commonmark.org">commonmark</a> nodes to line-delimited JSON:</p>
<pre><code class="javascript">var mkast = require('mkast')
  , ast = mkast.parse('# Title\n&lt;? @include file.md ?&gt;');
mkast.serialize(ast).pipe(process.stdout);
</code></pre>

<p>For more information see the <a href="#api">api docs</a>.</p>
<h2 id="api">API</h2>
<h3 id="parser">parser</h3>
<pre><code class="javascript">parser(stream[, cb])
</code></pre>

<p>Parse line-delimited JSON to vanilla objects.</p>
<p>When a callback function is given it is added as a listener for
the <code>error</code> and <code>finish</code> events on the parser stream.</p>
<p>Returns the parser stream.</p>
<ul>
<li><code>stream</code> Object input stream.</li>
<li><code>cb</code> Function callback function.</li>
</ul>
<h3 id="deserialize">deserialize</h3>
<pre><code class="javascript">deserialize([stream][, cb])
</code></pre>

<p>Deserialize line-delimited JSON to commonmark nodes.</p>
<p>When a callback function is given it is added as a listener for
the <code>error</code> and <code>eof</code> events on the deserializer stream.</p>
<p>The <code>eof</code> event can fire multiple times so the callback may be called
multiple times.</p>
<p>Returns the deserializer stream.</p>
<ul>
<li><code>stream</code> Object input stream.</li>
<li><code>cb</code> Function callback function.</li>
</ul>
<h3 id="serialize">serialize</h3>
<pre><code class="javascript">serialize(node[, opts][, cb])
</code></pre>

<p>Serialize a commonmark AST node to line-delimited JSON.</p>
<p>When the node is of the <code>document</code> type it's direct descendants are
...
with an <code>eof</code> type is sent to indicate the end of file (EOF).</p>
<p>When injecting documents into a stream it may be desirable to disable
this behaviour, to do so use:</p>
<pre><code class="javascript">{eof: false}
</code></pre>

<p>When a callback function is given it is added as a listener for
the <code>error</code> and <code>finish</code> events on the serializer stream.</p>
<p>Returns the serializer stream.</p>
<ul>
<li><code>node</code> Object input AST node.</li>
...
<p>MIT</p>
<hr />
<p>Created by <a href="https://github.com/mkdoc/mkdoc">mkdoc</a> on April 2, 2016</p>
...
Build Date UTC : 2020-02-26 16:54:25
-->
