...
<head>
  ...
  <script>
    // Current page data
    var mkdocs_page_name = "qs";
    var mkdocs_page_input_path = "node_modules\\qs\\README.md";
...
<pre><code class="javascript">var qs = require('qs');
...
<pre><code class="javascript">assert.deepEqual(qs.parse('foo[bar]=baz'), {
  foo: {
    bar: 'baz'
  }
});
...
<pre><code class="javascript">var nullObject = qs.parse('a[hasOwnProperty]=b', { plainObjects: true });
...
<pre><code class="javascript">var protoObject = qs.parse('a[hasOwnProperty]=b', { allowPrototypes: true });
assert.deepEqual(protoObject, { a: { hasOwnProperty: 'b' } });
</code></pre>

<p>URI encoded strings work too:</p>
<pre><code class="javascript">assert.deepEqual(qs.parse('a%5Bb%5D=c'), {
  a: { b: 'c' }
});
</code></pre>

<p>You can also nest your objects, like <code>'foo[bar][baz]=foobarbaz'</code>:</p>
<pre><code class="javascript">assert.deepEqual(qs.parse('foo[bar][baz]=foobarbaz'), {
  foo: {
    bar: {
      baz: 'foobarbaz'
    }
...
<pre><code class="javascript">var expected = {
  a: {
    b: {
      c: {
        d: {
...
<pre><code class="javascript">var deep = qs.parse('a[b][c][d][e][f][g][h][i]=j', { depth: 1 });
...
<pre><code class="javascript">var limited = qs.parse('a=b&amp;c=d', { parameterLimit: 1 });
assert.deepEqual(limited, { a: 'b' });
</code></pre>

<p>An optional delimiter can also be passed:</p>
<pre><code class="javascript">var delimited = qs.parse('a=b;c=d', { delimiter: ';' });
assert.deepEqual(delimited, { a: 'b', c: 'd' });
</code></pre>

<p>Delimiters can be a regular expression too:</p>
<pre><code class="javascript">var regexed = qs.parse('a=b;c=d,e=f', { delimiter: /[;,]/ });
assert.deepEqual(regexed, { a: 'b', c: 'd', e: 'f' });
</code></pre>

<p>Option <code>allowDots</code> can be used to enable dot notation:</p>
<pre><code class="javascript">var withDots = qs.parse('a.b=c', { allowDots: true });
...
<pre><code class="javascript">var withArray = qs.parse('a[]=b&amp;a[]=c');
assert.deepEqual(withArray, { a: ['b', 'c'] });
</code></pre>

<p>You may specify an index as well:</p>
<pre><code class="javascript">var withIndexes = qs.parse('a[1]=c&amp;a[0]=b');
...
their order:</p>
<pre><code class="javascript">var noSparse = qs.parse('a[1]=b&amp;a[15]=c');
assert.deepEqual(noSparse, { a: ['b', 'c'] });
</code></pre>

<p>Note that an empty string is also a value, and will be preserved:</p>
<pre><code class="javascript">var withEmptyString = qs.parse('a[]=&amp;a[]=b');
...
<pre><code class="javascript">var withMaxIndex = qs.parse('a[100]=b');
assert.deepEqual(withMaxIndex, { a: { '100': 'b' } });
</code></pre>

<p>This limit can be overridden by passing an <code>arrayLimit</code> option:</p>
<pre><code class="javascript">var withArrayLimit = qs.parse('a[1]=b', { arrayLimit: 0 });
assert.deepEqual(withArrayLimit, { a: { '1': 'b' } });
</code></pre>

<p>To disable array parsing entirely, set <code>parseArrays</code> to <code>false</code>.</p>
<pre><code class="javascript">var noParsingArrays = qs.parse('a[]=b', { parseArrays: false });
assert.deepEqual(noParsingArrays, { a: { '0': 'b' } });
</code></pre>

<p>If you mix notations, <strong>qs</strong> will merge the two items into an object:</p>
<pre><code class="javascript">var mixedNotation = qs.parse('a[0]=b&amp;a[b]=c');
assert.deepEqual(mixedNotation, { a: { '0': 'b', b: 'c' } });
</code></pre>

<p>You can also create arrays of objects:</p>
<pre><code class="javascript">var arraysOfObjects = qs.parse('a[][b]=c');
...
<pre><code class="javascript">var encoded = qs.stringify({ a: { b: 'c' } }, { encoder: function (str) {
...
<pre><code class="javascript">var decoded = qs.parse('x=z', { decoder: function (str) {
...
<pre><code class="javascript">qs.stringify({ a: ['b', 'c', 'd'] });
// 'a[0]=b&amp;a[1]=c&amp;a[2]=d'
</code></pre>

<p>You may override this by setting the <code>indices</code> option to <code>false</code>:</p>
<pre><code class="javascript">qs.stringify({ a: ['b', 'c', 'd'] }, { indices: false });
// 'a=b&amp;a=c&amp;a=d'
</code></pre>
...
<pre><code class="javascript">assert.equal(qs.stringify({ a: null, b: undefined }), 'a=');
</code></pre>

<p>The delimiter may be overridden with stringify as well:</p>
<pre><code class="javascript">assert.equal(qs.stringify({ a: 'b', c: 'd' }, { delimiter: ';' }), 'a=b;c=d');
</code></pre>
...
<pre><code class="javascript">var date = new Date(7);
...
<pre><code class="javascript">function alphabeticalSort(a, b) {
...
<pre><code class="javascript">function filterFunc(prefix, value) {
  if (prefix == 'b') {
...
<pre><code class="javascript">var withNull = qs.stringify({ a: null, b: '' });
...
<pre><code class="javascript">var equalsInsensitive = qs.parse('a&amp;b=');
...
<pre><code class="javascript">var strictNull = qs.stringify({ a: null, b: '' }, { strictNullHandling: true });
assert.equal(strictNull, 'a&amp;b=');
</code></pre>

<p>To parse values without <code>=</code> back to <code>null</code> use the <code>strictNullHandling</code> flag:</p>
<pre><code class="javascript">var parsedStrictNull = qs.parse('a&amp;b=', { strictNullHandling: true });
assert.deepEqual(parsedStrictNull, { a: null, b: '' });
</code></pre>

<p>To completely skip rendering keys with <code>null</code> values, use the <code>skipNulls</code> flag:</p>
<pre><code class="javascript">var nullsSkipped = qs.stringify({ a: 'b', c: null}, { skipNulls: true });
...
<pre><code class="javascript">var encoder = require('qs-iconv/encoder')('shift_jis');
var shiftJISEncoded = qs.stringify({ a: 'こんにちは！' }, { encoder: encoder });
assert.equal(shiftJISEncoded, 'a=%82%B1%82%F1%82%C9%82%BF%82%CD%81I');
</code></pre>

<p>This also works for decoding of query strings:</p>
<pre><code class="javascript">var decoder = require('qs-iconv/decoder')('shift_jis');
