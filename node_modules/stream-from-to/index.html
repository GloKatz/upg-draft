...
<head>
  ...
  <script>
    ...
    var mkdocs_page_input_path = "node_modules\\stream-from-to\\README.md";
  ...
  <hr/>
          ...
          <div role="main">
            <div class="section">
                ...
                <h1 id="stream-from-to">stream-from-to <a href="https://travis-ci.org/alanshaw/stream-from-to"><img alt="Build Status" src="https://travis-ci.org/alanshaw/stream-from-to.svg" /></a> <a href="https://david-dm.org/alanshaw/stream-from-to"><img alt="Dependency Status" src="https://david-dm.org/alanshaw/stream-from-to.svg" /></a></h1>
<p>Utility for piping to/from a stream from a variety of sources to a variety of destinations.</p>
<p>Pass stream-from-to a function that creates a <a href="https://npmjs.org/package/through">through</a> stream and it'll give you back an object that simplifies the constructon of source and destination streams for piping to/from the through stream.</p>
<h2 id="usage-examples">Usage examples</h2>
<pre><code class="javascript">var streamft = require(&quot;stream-from-to&quot;)
...
<p>With <a href="https://github.com/substack/brfs">brfs</a>:</p>
<pre><code class="javascript">var streamft = require(&quot;stream-from-to&quot;)
  , brfs = require(&quot;brfs&quot;)
...
<h2 id="api">API</h2>
<h3 id="frompathpath-opts">from.path(path, opts)</h3>
<p>Create a readable stream from <code>path</code> and pipe to the through stream. <code>path</code> can be a single path or array of paths.</p>
<h3 id="fromstringstring">from.string(string)</h3>
<p>Create a readable stream from <code>string</code> and pipe to the through stream. <code>string</code> can be a single string or array of strings.</p>
<h3 id="concatfrompathspaths-opts">concat.from.paths(paths, opts)</h3>
<p>Create and concatinate readable streams from <code>paths</code> and pipe to the through stream.</p>
<h3 id="concatfromstringsstrings-opts">concat.from.strings(strings, opts)</h3>
<p>Create and concatinate readable streams from <code>strings</code> and pipe to the through stream.</p>
<h3 id="topathpath-cb">to.path(path, cb)</h3>
<p>Create a writeable stream to <code>path</code> and pipe output from the through stream to it. <code>path</code> can be a single path, or array of output paths if you specified an array of inputs. The callback function <code>cb</code> will be invoked when data has finished being written.</p>
<h3 id="tobufferopts-cb">to.buffer(opts, cb)</h3>
<p>Create a <a href="https://npmjs.org/package/concat-stream">concat-stream</a> and pipe output from the through stream to it. The callback function <code>cb</code> will be invoked when the buffer has been created.</p>
<h3 id="tostringopts-cb">to.string(opts, cb)</h3>
<p>Create a <a href="https://npmjs.org/package/concat-stream">concat-stream</a> and pipe output from the through stream to it. The callback function <code>cb</code> will be invoked when the string has been created.</p>
