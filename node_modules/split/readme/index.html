...
<head>
  ...
  <script>
    // Current page data
    var mkdocs_page_name = "Split (matcher)";
    var mkdocs_page_input_path = "node_modules\\split\\readme.markdown";
  ...
  <hr/>
          ...
          <div role="main">
            <div class="section">
                ...
                <h1 id="split-matcher">Split (matcher)</h1>
<p><a href="http://travis-ci.org/dominictarr/split"><img alt="build status" src="https://secure.travis-ci.org/dominictarr/split.png" /></a></p>
...
<pre><code class="js">  fs.createReadStream(file)
    .pipe(split())
...
<p><code>split</code> takes the same arguments as <code>string.split</code> except it defaults to '/\r?\n/' instead of ',', and the optional <code>limit</code> parameter is ignored.
<a href="https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/String/split">String#split</a></p>
<p><code>split</code> takes an optional options object on its third argument.</p>
<pre><code class="js">  split(matcher, mapper, options)
</code></pre>

<p>Valid options:</p>
<ul>
<li>maxLength - The maximum buffer length without seeing a newline or <code>matcher</code>,
  if a single line exceeds this, the split stream will emit an error.</li>
</ul>
<pre><code class="js">  split(JSON.parse, null, { maxLength: 2})
</code></pre>

<ul>
<li>trailing - By default the last buffer not delimited by a newline or <code>matcher</code> will be emitted. To prevent this set <code>options.trailing</code> to <code>false</code>.</li>
</ul>
<pre><code class="js">  split(JSON.parse, null, { trailing: false })
</code></pre>

<h2 id="keep-matched-splitter">keep matched splitter</h2>
<p>As with <code>String#split</code>, if you split by a regular expression with a matching group,
the matches will be retained in the collection.</p>
<pre><code>stdin
.pipe(split(/(\r?\n)/))
... //lines + separators.
</code></pre>

<h1 id="ndj-newline-delimited-json">NDJ - Newline Delimited Json</h1>
<p><code>split</code> accepts a function which transforms each line.</p>
<pre><code class="js">fs.createReadStream(file)
