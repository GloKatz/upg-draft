...
<head>
  ...
  <title>Request - Simplified HTTP client - DCP</title>
  ...
  <script>
    // Current page data
    var mkdocs_page_name = "Request - Simplified HTTP client";
    var mkdocs_page_input_path = "node_modules\\request\\README.md";
...
<body class="wy-body-for-nav" role="document">
  ...
  <ul class="wy-breadcrumbs">
    ...
    <li>Request - Simplified HTTP client</li>
...
<pre><code class="js">request('http://google.com/doodle.png').pipe(fs.createWriteStream('doodle.png'))
</code></pre>
...
<pre><code class="js">fs.createReadStream('file.json').pipe(request.put('http://mysite.com/obj.json'))
</code></pre>
...
<pre><code class="js">request.get('http://google.com/img.png').pipe(request.put('http://mysite.com/img.png'))
</code></pre>
...
<pre><code class="js">http.createServer(function (req, resp) {
...
<pre><code class="js">req.pipe(request('http://mysite.com/doodle.png')).pipe(resp)
</code></pre>

<p>Also, none of this new functionality conflicts with requests previous features, it just expands them.</p>
<pre><code class="js">var r = request.defaults({'proxy':'http://localproxy.com'})
...
<pre><code class="js">request.post('http://service.com/upload', {form:{key:'value'}})
// or
...
<pre><code class="js">var formData = {
...
<pre><code class="js">// NOTE: Advanced use-case, for normal use see 'formData' usage above
...
<pre><code class="js">request.get('http://some.server.com/').auth('username', 'password', false);
// or
...
<pre><code class="js">var username = 'username',
    password = 'password',
...
<pre><code class="js">// OAuth1.0 - 3-legged server side flow (Twitter example)
// step 1
...
* Pass <code>signature_method : 'RSA-SHA1'</code>
* Instead of <code>consumer_secret</code>, specify a <code>private_key</code> string in
...
the following changes to the OAuth options object:
* Pass <code>signature_method : 'PLAINTEXT'</code></p>
...
* Pass <code>transport_method : 'query'</code> or <code>transport_method : 'body'</code> in the OAuth
  options object.
* <code>transport_method</code> defaults to <code>'header'</code></p>
...
* Manually generate the body hash and pass it as a string <code>body_hash: '...'</code>
* Automatically generate the body hash by passing <code>body_hash: true</code></p>
<p><a href="#table-of-contents">back to top</a></p>
<hr />
...
<pre><code class="js">/* Pattern */ 'http://unix:SOCKET:PATH'
...
<h3 id="using-optionsagentoptions">Using <code>options.agentOptions</code></h3>
...
<hr />
<ul>
<li><code>time</code> - If <code>true</code>, the request-response cycle (including all redirects) is timed at millisecond resolution. When set, the following properties are added to the response object:</li>
...
<li>
<p><code>timingPhases</code> Contains the durations of each request phase. If there were redirects, the properties reflect the timings of the final request in the redirect chain:</p>
<ul>
...
<p>For example:</p>
<pre><code class="js">//requests using baseRequest() will set the 'x-token' header
...
<pre><code class="js">request.cookie('key1=value1')
</code></pre>
...
<pre><code class="js">request.get('http://10.255.255.1', {timeout: 1500}, function(err) {
...
<pre><code class="js">var request = request.defaults({jar: true})
...
<pre><code class="js">var j = request.jar()
...
<pre><code class="js">var FileCookieStore = require('tough-cookie-filestore');
