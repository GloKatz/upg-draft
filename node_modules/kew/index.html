...
<head>
  ...
  <script>
    ...
    var mkdocs_page_input_path = "node_modules\\kew\\README.md";
  ...
  <hr/>
          ...
          <div role="main">
            <div class="section">
                ...
                <p>==================================</p>
<p><a href="https://travis-ci.org/Medium/kew"><img alt="Build Status" src="https://travis-ci.org/Medium/kew.svg" /></a></p>
...
<h2 id="a-few-answers-for-a-few-questions">A few answers (for a few questions)</h2>
...
<pre><code class="javascript">// assuming the getUrlContent() function exists and retrieves the content of a url
...
<h2 id="how-do-i-use-kew">How do I use <strong>kew</strong>?</h2>
<p>As a precursor to all the examples, the following code must be at the top of your page:</p>
<pre><code class="javascript">var Q = require('kew')
</code></pre>

<h3 id="convert-a-literal-into-a-promise">Convert a literal into a promise</h3>
...
<pre><code class="javascript">// create a promise which passes a value to the next then() call
...
<pre><code class="javascript">// create the deferreds
...
<pre><code class="javascript">// create the deferred
var defer = Q.defer()
...
<h3 id="handling-successful-results-with-then">Handling successful results with <code>.then()</code></h3>
<p>When a promise is resolved, you may call the <code>.then()</code> method to retrieve the value of the promise:</p>
<pre><code class="javascript">promise.then(function (result) {
...
<pre><code class="javascript">Q.resolve('a')
...
<pre><code class="javascript">var htmlPromise = getUrlContent(myUrl)
...
<h3 id="handling-errors-with-fail">Handling errors with <code>.fail()</code></h3>
<p>If a promise is rejected for some reason, you may handle the failure case with the <code>.fail()</code> function:</p>
<pre><code class="javascript">getObjectPromise
  .fail(function (e) {
...
<h3 id="fin-when-things-are-finished"><code>.fin()</code> when things are finished</h3>
...
<pre><code class="javascript">var connection = db.connect()
...
<h2 id="other-utility-methods">Other utility methods</h2>
<h3 id="all-for-many-things"><code>.all()</code> for many things</h3>
...
<pre><code class="javascript">var promises = []
...
<h3 id="delay-for-future-promises"><code>.delay()</code> for future promises</h3>
...
<pre><code class="javascript">getUrlContent(url1)
.fail(function () {
...
<pre><code class="javascript">Q.delay(obj, 20).then(function (result) {
  console.log(result) // logs `obj` after 20ms
})
</code></pre>

<h3 id="fcall-for-delaying-a-function-invocation-until-the-next-tick"><code>.fcall()</code> for delaying a function invocation until the next tick:</h3>
<pre><code class="javascript">// Assume someFn() is a synchronous 2 argument function you want to delay.
...
<h3 id="ncall-and-nfcall-for-nodejs-callbacks"><code>.ncall()</code> and <code>.nfcall()</code> for Node.js callbacks</h3>
<p><code>Q.nfcall()</code> can be used to convert node-style callbacks into promises:</p>
<pre><code class="javascript">Q.nfcall(fs.writeFile, '/tmp/myFile', 'content')
  .then(function () {
...
<pre><code class="js">Q.ncall(redis.del, redis, 'my-key')
  .then(function () { return true })
  .fail(function () { return false })
</code></pre>

<h3 id="spread-for-arrays-of-promises"><code>.spread()</code> for arrays of promises</h3>
<p><code>()</code> can be used to convert node-style callbacks into promises:</p>
<pre><code class="javascript">Q.nfcall(function () {
...
<p><a href="https://github.com/azulus">Jeremy Stanley</a>
supported by
